...
public class FixedUniformStorage<T extends DynamicUniformStorage.DynamicUniform> {
    ...
    public FixedUniformStorage(String name, int blockSize, int capacity) {
        ...
        this.blockSize = Mth.roundToward((int)blockSize, (int)gpuDevice.getUniformOffsetAlignment());
        this.capacity = capacity;
        int alignedCapacity = Mth.smallestEncompassingPowerOfTwo((int)capacity);
        this.size = 0;
    ...
    public GpuBufferSlice write(T value) {
        ...
        GpuBufferSlice slice = this.buffer.currentBuffer().slice((long)i, (long)this.blockSize);
    ...
    public GpuBufferSlice[] writeAll(T[] values) {
        ...
        try (GpuBuffer.MappedView mappedView = RenderSystem.getDevice().createCommandEncoder().mapBuffer(ubo.slice((long)i, (long)(values.length * this.blockSize)), false, true);){
