...
public final class ToggleAllPlantTypesComponent
extends Component {
    private static final ClickGui GUI = WURST.getGui();
    private static final class_327 TR = ToggleAllPlantTypesComponent.MC.field_1772;
    ...
    public void render(class_332 context, int mouseX, int mouseY, float partialTicks) {
        ...
        if (hIcon || hName) {
            GUI.setTooltip(String.valueOf(WText.translated("gui.wurst.autofarm.all_plant_types", new Object[0])));
        } else if (hHarvest) {
        ...
        Boolean harvestingEnabled = this.setting.isHarvestingEnabled();
        if (harvestingEnabled == Boolean.TRUE) {
            ...
            ClickGuiIcons.drawIndeterminateCheck(context, x3, y3, x4, y2, hHarvest, false);
        }
        Boolean replantingEnabled = this.setting.isReplantingEnabled();
        if (replantingEnabled == Boolean.TRUE) {
            ...
            ClickGuiIcons.drawIndeterminateCheck(context, x5, y3, x6, y2, hReplant, false);
        }
